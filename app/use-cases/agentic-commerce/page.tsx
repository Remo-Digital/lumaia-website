      ...
      <main id="main-content" className="pt-24">
            ...
            <div className="glass-strong rounded-2xl p-12 mb-8 flex items-center justify-center min-h-[250px] relative overflow-hidden">
              <div className="absolute inset-0 opacity-20" aria-hidden="true">
                <svg className="w-full h-full" viewBox="0 0 600 250" fill="none">
                  {/* Animated AI agent shopping visualization */}
              ...
              <span className="relative text-white/20 text-sm" aria-label={ac.heroAlt}>{ac.heroAlt}</span>
            </div>

            {/* Direct answer */}
            <div className="glass-accent rounded-2xl p-6 mb-12">
              <p className="text-white/70 text-base leading-relaxed">{ac.directAnswer}</p>
            </div>

            {/* Table of contents */}
            <nav className="glass rounded-2xl p-6 mb-16" aria-label={locale === 'de' ? 'Inhaltsverzeichnis' : 'Table of contents'}>
              <h2 className="text-white/60 text-sm font-semibold tracking-[0.15em] uppercase mb-4">{locale === 'de' ? 'Inhaltsverzeichnis' : 'Contents'}</h2>
              <ol className="space-y-2">
                {ac.toc.map((item, i) => (
                  <li key={i}>
                    <a href={`#section-${i + 1}`} className="text-white/50 text-sm hover:text-accent transition-colors">
                      {i + 1}. {item}
                    </a>
                  </li>
                ))}
        ...
        <article className="relative py-4 px-6" style={{ background: '#080614' }}>
          ...
          <div className="relative z-10 max-w-3xl mx-auto space-y-20">
            ...
            {/* Section 1: How agents shop */}
            <section id="section-1" className="scroll-mt-24">
              <h2 className="font-serif text-3xl md:text-4xl text-white mb-6">{ac.sections.howAgentsShop.title}</h2>
              <p className="text-white/55 text-base leading-relaxed mb-6">{ac.sections.howAgentsShop.intro}</p>
              <p className="text-white/55 text-base leading-relaxed mb-6">{ac.sections.howAgentsShop.protocolsIntro}</p>
              <div className="space-y-4 mb-8">
                {ac.sections.howAgentsShop.protocols.map(p => (
                  <div key={p.name} className="glass rounded-xl p-5">
                    <h3 className="text-accent font-semibold text-sm mb-1">{p.name} <span className="text-white/40 font-normal">({p.full})</span></h3>
                    <p className="text-white/50 text-sm leading-relaxed">{p.desc}</p>
                  </div>
                ))}
              </div>

              {/* Protocol diagram placeholder */}
              <div className="glass-strong rounded-2xl p-10 mb-8 flex items-center justify-center min-h-[180px] relative overflow-hidden">
                ...
                <span className="relative text-white/20 text-xs">{locale === 'de' ? 'Protokoll-\u00dcbersicht' : 'Protocol Overview'}</span>
              </div>

              <p className="text-white/60 text-base leading-relaxed font-medium">{ac.sections.howAgentsShop.conclusion}</p>
            </section>
            ...
            <section id="section-2" className="scroll-mt-24">
              ...
              <div className="space-y-4 text-white/55 text-base leading-relaxed">
                ...
                <p className="text-white/70 font-medium">{ac.sections.whyNow.p4}</p>
              </div>
            </section>
            ...
            <section id="section-3" className="scroll-mt-24">
              ...
              <p className="text-white/55 text-base leading-relaxed mb-6">{ac.sections.selfCheck.intro}</p>
              ...
              <p className="text-white/60 text-base leading-relaxed italic">{ac.sections.selfCheck.conclusion}</p>
            </section>
            ...
            <section id="section-4" className="scroll-mt-24">
              ...
              <div className="space-y-4 text-white/55 text-base leading-relaxed">
                ...
                <p className="text-white/70 font-medium italic">{ac.sections.examples.p3}</p>
              </div>
            </section>
            ...
            <section id="section-5" className="scroll-mt-24">
              ...
              <p className="text-white/55 text-base leading-relaxed mb-6">{ac.sections.prerequisites.intro}</p>
              <ol className="space-y-4 mb-8" role="list">
                {ac.sections.prerequisites.items.map((item, i) => (
                  <li key={i} className="glass rounded-xl p-5 flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-white/80 font-semibold text-base mb-1">{item.title}</h3>
                      <p className="text-white/50 text-sm leading-relaxed">{item.desc}</p>
                    </div>
                  </li>
                ))}
              </ol>
              <p className="text-white/55 text-base leading-relaxed">{ac.sections.prerequisites.conclusion}</p>
            </section>
                  ...
                  <table className="w-full text-left" role="table">
                    <thead>
                      <tr className="border-b border-white/10">
                        <th className="px-6 py-4 text-sm font-semibold text-white/40" scope="col">{locale === 'de' ? 'Merkmal' : 'Attribute'}</th>
                        <th className="px-6 py-4 text-sm font-semibold text-white/60" scope="col">{locale === 'de' ? 'Klassisch' : 'Traditional'}</th>
                    ...
                    <tbody>
                      {ac.sections.comparisonTable.rows.map((row, i) => (
                        <tr key={i} className={i < ac.sections.comparisonTable.rows.length - 1 ? 'border-b border-white/5' : ''}>
                          ...
                          <td className="px-6 py-4 text-white/40 text-sm">{row.traditional}</td>
            ...
            <section className="glass-strong rounded-2xl p-8 mb-8">
              ...
              <p className="text-white/55 text-base leading-relaxed">{ac.conclusion}</p>
            </section>
            ...
            <section className="glass rounded-2xl p-6 flex items-start gap-5">
              <div className="flex-shrink-0 w-16 h-16 rounded-full bg-gradient-to-br from-accent/20 to-purple/20 flex items-center justify-center text-white/30 text-lg font-semibold" aria-hidden="true">
                RG
              </div>
              <div>
                <h3 className="text-white/80 font-semibold text-base">{ac.author.name}</h3>
                <p className="text-accent/70 text-sm mb-2">{ac.author.role}</p>
                <p className="text-white/45 text-sm leading-relaxed">{ac.author.bio}</p>
              </div>
            </section>
          </div>
        </article>
