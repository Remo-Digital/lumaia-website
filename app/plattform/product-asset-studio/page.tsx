      ...
      <main id="main-content" className="pt-24">
        ...
        <section className="relative overflow-hidden py-24 px-6" style={{ background: 'linear-gradient(135deg, #03020a 0%, #0d0822 45%, #04021a 100%)' }}>
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            <div className="glass-strong rounded-2xl p-10 mb-8 min-h-[250px] relative overflow-hidden flex items-center justify-center">
              ...
              <span className="relative text-white/15 text-xs">{pas.heroAlt}</span>
            </div>

            <p className="text-white/60 text-lg leading-relaxed mb-8">{pas.directOffer}</p>
            ...
            <p className="text-white/40 text-sm mb-8">{pas.ctaMicro}</p>

            {/* Trust layer */}
            <div className="glass rounded-2xl p-6">
              <p className="text-white/45 text-sm leading-relaxed">{pas.trustLayer}</p>
            </div>
          </div>
        </section>
        ...
        <article className="relative py-4 px-6" style={{ background: '#080614' }}>
          ...
          <div className="relative z-10 max-w-3xl mx-auto space-y-20 py-16">
            ...
            <section>
              ...
              <p className="text-white/55 text-base leading-relaxed mb-4">{pas.sections.whyDifferent.p1}</p>
              <p className="text-white/55 text-base leading-relaxed mb-6">{pas.sections.whyDifferent.p2}</p>
            ...
            <section>
              ...
              <p className="text-white/55 text-base leading-relaxed">{pas.sections.whatIs.desc}</p>
            </section>

            {/* 4 Asset types */}
            <section>
              <h2 className="font-serif text-3xl md:text-4xl text-white mb-4">{pas.sections.assetTypes.title}</h2>
              <p className="text-white/55 text-base leading-relaxed mb-8">{pas.sections.assetTypes.intro}</p>
                ...
                {pas.sections.assetTypes.types.map((type, i) => (
                  <article key={i} className="glass glow-hover rounded-2xl overflow-hidden transition-all duration-300">
                    <div className="h-32 bg-gradient-to-br from-accent/5 to-purple/5 flex items-center justify-center">
                      <span className="text-white/15 text-xs">{type.title} Placeholder</span>
                    </div>
                    <div className="p-6">
            ...
            <section>
              ...
              <p className="text-white/55 text-base leading-relaxed mb-4">{pas.sections.googleStandard.intro}</p>
              <div className="glass-accent rounded-xl p-5 mb-6">
                <p className="text-white/70 text-base leading-relaxed font-medium">{pas.sections.googleStandard.prio1}</p>
              </div>
              <div className="space-y-3">
                {pas.sections.googleStandard.otherGroups.map((g, i) => (
                  <div key={i} className="glass rounded-xl p-4">
                    <h3 className="text-white/60 text-sm font-semibold mb-1">{g.title}</h3>
              ...
              <ol className="space-y-4">
                {pas.sections.howItWorks.steps.map((step, i) => (
                  <li key={i} className="glass rounded-xl p-5 flex items-start gap-4">
                    ...
                    <p className="text-white/60 text-base leading-relaxed pt-1">{step}</p>
                  </li>
                ))}
              </ol>
            </section>
            ...
            <section>
              ...
              <p className="text-white/55 text-base leading-relaxed mb-4">{pas.sections.example1.before}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="glass rounded-xl p-8 flex items-center justify-center min-h-[150px]">
                  <span className="text-white/15 text-xs">{locale === 'de' ? 'Vorher: 1 Bild' : 'Before: 1 image'}</span>
                </div>
              ...
              <p className="text-white/50 text-base leading-relaxed">{pas.sections.example1.after}</p>
            </section>

            <section>
              <h2 className="font-serif text-3xl md:text-4xl text-white mb-6">{pas.sections.example2.title}</h2>
              <p className="text-white/55 text-base leading-relaxed mb-4">{pas.sections.example2.cutout}</p>
              <p className="text-white/55 text-base leading-relaxed mb-4">{pas.sections.example2.upscaling}</p>
              <p className="text-white/40 text-sm italic">{pas.sections.example2.note}</p>
            </section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {pas.sections.results.items.map((item, i) => (
                  <div key={i} className="glass rounded-2xl p-5 text-center">
                    ...
                    <div className="text-white/50 text-xs leading-tight">{item.label}</div>
                  </div>
                ))}
              </div>
              <p className="text-white/70 text-lg font-medium text-center">{pas.sections.results.conclusion}</p>
            </section>

            {/* Audience */}
            <section>
              <h2 className="font-serif text-3xl md:text-4xl text-white mb-6">{pas.sections.audience.title}</h2>
              <p className="text-white/55 text-base leading-relaxed mb-4">{pas.sections.audience.forWhom}</p>
              <p className="text-white/40 text-sm italic">{pas.sections.audience.notFor}</p>
            </section>
          </div>
        </article>
        ...
        <section className="py-20 px-6" style={{ background: 'linear-gradient(135deg, #03020a 0%, #0d0822 45%, #04021a 100%)' }}>
          <div className="max-w-2xl mx-auto text-center">
            ...
            <p className="text-white/40 text-sm mt-4">{pas.ctaMicro}</p>
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4 text-sm">
              <a href={localizedHref('/use-cases/agentic-commerce', locale)} className="text-white/50 hover:text-accent transition-colors">Use Case: Agentic Commerce</a>
              <span className="text-white/20 hidden sm:inline" aria-hidden="true">&bull;</span>
              <a href={localizedHref('/pricing', locale)} className="text-white/50 hover:text-accent transition-colors">{locale === 'de' ? 'Preise' : 'Pricing'}</a>
              <span className="text-white/20 hidden sm:inline" aria-hidden="true">&bull;</span>
              <a href={localizedHref('/plattform', locale)} className="text-white/50 hover:text-accent transition-colors">{locale === 'de' ? 'Plattform-\u00dcbersicht' : 'Platform Overview'}</a>
            </div>
          </div>
        </section>
      </main>
